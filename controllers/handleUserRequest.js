...
export const handlePatientRequest = async (req, res) => {
  try {
    const { symptom, userLon, userLat } = req.body;

    // Validation - Updated to check for coordinates instead of string location
    if (!symptom || !userLon || !userLat) {
      return res.status(400).json({
        status: "error", 
        error: "Please provide 'symptom', 'userLon', and 'userLat' in the request body.",
      });
    }

    const intentAnalysis = await classifyUserIntent(symptom);

    // --- CASE 1: Emergency Detected ---
    ...
    const retrievedHospitals = await Hospital.find({
      // First, find hospitals within a 5 km (5000 meters) radius
      location: {
        $near: {
          $geometry: {
            type: "Point",
      ...
      // Then, filter those nearby hospitals by the required symptom
      $text: { $search: symptom },
    });
    ...
    if (retrievedHospitals.length === 0) {
      return res.status(200).json({
        ...
        botResponse: `I couldn't find any specialized hospitals for "${symptom}" near your location in our current database. Please consult a general physician nearby.`,
      });
    }
    ...
    const systemPrompt = `
      ...
      A user is reporting the following symptom: "${symptom}".
      
      I have queried our database and found the following verified hospitals near them:
    ...
    return res.status(200).json({
      status: "success",
      responseType: "MILD",
      userQuery: { symptom, coordinates: [userLon, userLat] },
      retrievedData: retrievedHospitals, 
